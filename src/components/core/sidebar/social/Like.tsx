...
const Like = ({ pageURL }) => {
    ...
    return (
        <button disabled={disabled()} onClick={click} title={liked() ? `${likes()} 人已点赞` : "Like"} class={`:: hover:text-rose-500 trans-linear h-15 w-24 font-sitetitle ${liked() ? " text-rose-500" : ""} ${animate() ? "animated animated-pulse animated-faster" : ""} ${disabled() ? " cursor-not-allowed" : ""}`}>
            {liked() ?
