...
const ToC = ({ toc, slug }: ToCInterface) => {
    ...
    return (
        <>
            <aside class=":: lg:max-w-80 lg:ml-6 <lg:fixed <lg:z-10 ">
                <div class=":: lg:top-10 md:sticky ">
                    ...
                    <div id="toc" class={`:: duration-200 z-20 overflow-y-auto transition-max-height toc-modal ${visible() ? '<lg:max-h-42vh' : '<lg:max-h-0'} `}>
                        <div class=":: h-4 "></div>
                        <div class=":: ml-2 flex font-headline "><label>{LL().sidebar.TOC()}</label> <span class=":: mt-1px ">（{readingProgress()}%）</span></div>
            ...
            <script innerHTML={`
            const sections = document.querySelectorAll("article h2, h3, h4");
            ...
            if (entry.isIntersecting) {
                tocLinks.forEach(link => {
                    ...
                    if (link.getAttribute("href").substring(1) === entry.target.id) {
                        link.classList.add("active"); console.log("kjkffj")
                    }
                });
            }
        });
